...
public class DESUtils {
    ...
    private static String private_key = "galaxyDes";

    public static String encrypt(String str) {
        return new DESUtils(private_key).encryptStr(str);
    }

    public static String decrypt(String strMi) {
        return new DESUtils(private_key).decryptStr(strMi);
    }
    ...
    public static void main(String[] args) throws Exception {
        if (null == args || args.length == 0) {
            System.out.println("请输入密码参数！");
            return;
        }
        String str1 = args[0];
        // DES 加密字符串
        String str2 = DESUtils.encrypt(str1);
        // DES 解密字符串
        String deStr = DESUtils.decrypt(str2);
