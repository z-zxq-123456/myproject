...
public class ServiceConfig<T> extends AbstractServiceConfig {
    ...
    private volatile static Protocol protocol = null;
            ...
            if (! Constants.SCOPE_LOCAL.toString().equalsIgnoreCase(scope) ){
                ...
                if (registryURLs != null && registryURLs.size() > 0
                    ...
                    for (URL registryURL : registryURLs) {
                        ...
                        Invoker<?> invoker = fetchProxyFactory().getInvoker(ref, (Class) interfaceClass, registryURL.addParameterAndEncoded(Constants.EXPORT_KEY, url.toFullString()));

                        Exporter<?> exporter = fetchRefProtocol().export(invoker);
                        exporters.add(exporter);
                    }
                } else {
                    Invoker<?> invoker = fetchProxyFactory().getInvoker(ref, (Class) interfaceClass, url);
    ...
    private void exportLocal(URL url) {
        if (!Constants.LOCAL_PROTOCOL.equalsIgnoreCase(url.getProtocol())) {
            URL local = URL.valueOf(url.toFullString())
                    ...
                    .setPort(0);
            Exporter<?> exporter = fetchRefProtocol().export(
                    fetchProxyFactory().getInvoker(ref, (Class) interfaceClass, local));
            exporters.add(exporter);
