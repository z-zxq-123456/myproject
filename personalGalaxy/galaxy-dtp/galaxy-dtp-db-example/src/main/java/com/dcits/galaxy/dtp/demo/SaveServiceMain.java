...
import com.dcits.galaxy.dtp.annotation.SubmitLogAspect;
...
public class SaveServiceMain {
	...
	public static void main(String[] args) throws Exception {
		...
		System.setProperty("galaxy.protocol.port", "20881");
		System.setProperty("galaxy.application.group", "Save_Group");
		System.setProperty("galaxy.application.name", "Save_Service");
		...
		//初始化事务配置
		Sequences sequences = SpringApplicationContext.getContext().getBean(Sequences.class);
		sequences.setServerId("save");
...
//		branchResumer.start();
		...
		// 服务提供者暴露服务配置
		ServiceConfig<SaveService> service = new ServiceConfig<SaveService>(); // 此实例很重，封装了与注册中心的连接，请自行缓存，否则可能造成内存和连接泄漏
		service.setApplication(application);
		service.setRegistry(registry); // 多个注册中心可以用setRegistries()
		service.setProtocol(protocol); // 多个协议可以用setProtocols()
		service.setInterface(SaveService.class);
		SaveService saveService = (SaveService) SpringApplicationContext.getContext().getBean("saveServiceImpl");
		service.setRef( saveService );
//		service.setVersion("1.0.0");
		
		// 暴露及注册服务
		service.export();
		System.out.println("SaveService 向 zookeeper 注册成功");
