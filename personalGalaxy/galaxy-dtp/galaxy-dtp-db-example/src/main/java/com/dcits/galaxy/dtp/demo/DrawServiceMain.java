...
public class DrawServiceMain {
	...
	public static void main(String[] args) throws Exception {
		...
		System.setProperty("galaxy.protocol.port", "20882");
		System.setProperty("galaxy.application.group", "Draw_Group");
		System.setProperty("galaxy.application.name", "Draw_Service");
...
//		branchResumer.start();
		...
		// 服务提供者暴露服务配置
		ServiceConfig<DrawService> service = new ServiceConfig<DrawService>(); // 此实例很重，封装了与注册中心的连接，请自行缓存，否则可能造成内存和连接泄漏
		service.setApplication(application);
		service.setRegistry(registry); // 多个注册中心可以用setRegistries()
		service.setProtocol(protocol); // 多个协议可以用setProtocols()
		service.setInterface(DrawService.class);
		DrawService drawService = (DrawService) SpringApplicationContext.getContext().getBean("drawServiceImpl");
		service.setRef( drawService );
//		service.setVersion("1.0.0");
		
		// 暴露及注册服务
		service.export();
		System.out.println("DrawService 向 zookeeper 注册成功");
