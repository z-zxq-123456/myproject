...
/**
 * 封装 分布式缓存 全局参数表+主键分别在缓存中建索引
 * @author tangxlf
 * @date   2014-01-20
 * 
 */
@Repository
public class KeyIndexParaCache implements IParaCache{	 
	   @Resource
	   ...
	   private static final Logger log = LoggerFactory.getLogger(KeyIndexParaCache.class);	   
	  /**
	   * 初始化装载
	  ...
	  public synchronized void initLoad(String tableName){
		  if(!paraCacheHAO.isInit(tableName)){//如果没有初始化
		    	 ...
		    	 for(int i=0;i<dataList.size();i++){
		    		 Map<String,Object> dbRowMap = ForamtMapUtil.MapToHump(dataList.get(i));
		    		 paraCacheHAO.insertRow(tableName,dbRowMap);
		    		 paraCacheHAO.insertSigleKeyRowIndex(tableName,dbRowMap);
				 }
		  }		   
	  }
	  /**
	...
	public List<Map<String, Object>>  getList(String tableName,String pkfield,Object pkValue){
		 ...
		 try {	
			 initLoad(tableName);	
			 rowList = paraCacheHAO.getList(tableName, pkfield, pkValue);
		} catch (Throwable t) {		
			log.error(t.getMessage());
			return paraDbHAO.getList(tableName, pkfield, pkValue);			
		}
		return rowList;
	  }	  
      /**
	...
	@Transactional
	   public  int  insertRow(String tableName,Map<String,Object> rowMap){		  
		   ...
		   paraCacheHAO.insertRow(tableName,rowMap);
		   paraCacheHAO.insertSigleKeyRowIndex(tableName,rowMap);
		   return  insertRowInt ;
	   }
	   /**
	    * 同步修改缓存和数据库记录
	...
	@Transactional
	   public  int  deleteRow(String tableName,Map<String,Object> keyMap){
		   ...
		   paraCacheHAO.deleteSigleKeyRowIndex(tableName,keyMap);
		   return  deleteRowInt ;
	   }
