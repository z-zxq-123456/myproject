...
var rowData;
    $(document).ready(function () {
        var rowData;
        if (parent.$("#earlyConfigList").find(".selected").length == 1) {
            rowData = parent.$('#earlyConfigList').DataTable().rows(".selected").data()[0];
            ...
            if (rowData.configKey.indexOf('port') > 0) {
                var dataA = [{"id": "0010001", "text": "应用单实例修改"}];
                $("#updateType").select2({
                    data: dataA
                });
            } else {
                var dataA = [{"id": "0010001", "text": "应用单实例修改"},{"id": "0010002", "text": "应用修改"}];
                $("#updateType").select2({
                    data: dataA
                });
            }
    ...
    function dataEdit() {
        var url = contextPath + "/earlyUpdateEcmAppConfig";
        var rowData = parent.$('#earlyConfigList').DataTable().rows(".selected").data()[0];
        sendPostRequest(url, {
            appIntantId:parent.$("#appIntantId").val(),
