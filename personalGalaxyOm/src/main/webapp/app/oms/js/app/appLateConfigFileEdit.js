...
var rowData;
    $(document).ready(function () {
        var rowData;
        if (parent.$("#lateConfigList").find(".selected").length == 1) {
            rowData = parent.$('#lateConfigList').DataTable().rows(".selected").data()[0];
            ...
            if (rowData.configKey.indexOf('port') > 0) {
                var dataA = [{"id": "0011001", "text": "应用单实例修改"}];
                $("#updateType").select2({
                    data: dataA
                });
            } else {
                var dataA = [{"id": "0011001", "text": "应用单实例修改"},{"id": "0011002", "text": "应用修改"}];
                $("#updateType").select2({
                    data: dataA
                });
            }
    ...
    function dataEdit() {
        var url = contextPath + "/lateUpdateEcmAppConfig";
        var rowData = parent.$('#lateConfigList').DataTable().rows(".selected").data()[0];
        sendPostRequest(url, {
  ...
  $("#updateConfigValue").onblur(function() {
