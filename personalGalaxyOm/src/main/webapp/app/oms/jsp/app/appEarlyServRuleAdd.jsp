...
<form method="post" class="form form-horizontal" id="selectForm" name="selectForm">
    <div class="row">
        ...
        <div class="span2">
        <a class="button-select S " onclick="searchEarlyRule()"><i class="iconfont icon-4" ></i>搜索</a>
        </div>
    </div>
</form>
...
var appId =<%=appId %>;
 $(document).ready(function () {
        getPkList({
            url: contextPath + "/findParaCombox?paraParentId=0012&&isDefault=false",
            id: "appSersource",
            async: false
        });
        // 获取默认opt配置
        ...
        opt.ajax = {
            "url": contextPath + "/findEarlyAppValRule?appId=<%=appId %>",
            "type": "POST"
        };
        opt.columns = [
            {"data": "appSerName"},
...
function searchEarlyRule() {
      ...
      if(appSerClassName==""&&appSersource==""){
          api.ajax.url(contextPath + "/findEarlyAppValRule?appId="+appId ).load();
      }else{
           if(appSerClassName!=""){
               if(appSersource!=""){
                  api.ajax.url(contextPath + "/findEarlyAppValRule?appId=" + appId+"&appSersource="+appSersource+"&appSerClassName="+appSerClassName).load();
               }else{
                   api.ajax.url(contextPath + "/findEarlyAppValRule?appId=" + appId+"&appSerClassName="+appSerClassName).load();
               }
           }else{
               api.ajax.url(contextPath + "/findEarlyAppValRule?appId=" + appId+"&appSersource="+appSersource).load();
           }
      }
}
...
function addServRuleInfo() {
     ...
     if(rowSelect.length!=0){
          ...
          $.post(contextPath + '/saveEarlyValrule',obj,function(result){
