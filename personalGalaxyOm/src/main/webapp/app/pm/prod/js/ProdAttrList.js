...
$(document).ready(function() {
	...
	opt.autoWidth=false;
	opt.ajax= {
			 "url": contextPath+"/prodType/getAll",
			 "type": "POST"
       	 };
	opt.columns=[
	...
	drawDataTable($("#prodList"),opt);
     dataTableUi("prodList",["查询","添加","修改","删除","查看差异数据","刷新","返回"],["select","add","edit","delete","contrast","refresh","return"]);
       $("#add").hide();
       $("#edit").hide();
       $("#delete").hide();
       $("#select").hide();
...
 	$('#prodList tbody').on('click', 'tr', function (e) {
       mb_prod_edit('查看','ProdAttrList1.jsp','800','500');
...
function selectAll(){
    ...
    var api = prodTab.api();
    api.ajax.url(contextPath+"/prodType/getAll").load();
}
...
function mb_prod_contrast(){
    ...
    api.ajax.url(contextPath+"/prodType/getAllContrast").load();
}
