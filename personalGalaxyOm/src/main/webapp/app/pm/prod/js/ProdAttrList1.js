    ...
    getPkList({
      ...
      async: false
        ...
        var url = contextPath + "/prodDefine/getByProdType";
        if(contrastFlag){
            url = contextPath + "/prodDefine/getByProdTypeContrast";
        }
        opt.ajax= {
                 "url": url,
                 "type": "POST",
           ...
           //子行数据函数
             function format (d) {  //d是当前行的数据
                 ...
                 if(d.assembleType == "ATTR"){
                    ...
                    var childEventUrl = contextPath+"/prodDefine/selectChildrenEvent";
                    if(contrastFlag){
                        childEventUrl = contextPath+"/prodDefine/selectChildrenEventContrast";
                    }
                     $.ajax({
                         url: childEventUrl,
                         data:{
                         ...
                         success :function(resultGroup){
                             ...
                             var childPartUrl = contextPath+"/eventAttr/selectChildrenPart";
                             if(contrastFlag){
                                childPartUrl = contextPath+"/eventAttr/selectChildrenPartContrast";
                             }
                             for(n in result){
                                 var num=Number(n)+1;
